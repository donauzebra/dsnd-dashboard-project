...
def db_path():
    ...
    return (
        project_root / "python-package" / "employee_events"
        / "employee_events.db"
    )
...
def test_employee_table_exists(table_names):
    ...
    assert table_names.count("employee") > 0
...
def test_team_table_exists(table_names):
    ...
    # is in the table_names list
    assert table_names.count("team") > 0
...
def test_employee_events_table_exists(table_names):
    ...
    # is in the table_names list
    assert table_names.count("employee_events") > 0
